...
public class CFGTest {
    ...
    @Test
    public void addNode_duplicate2() {
        int initialSize = cfg.nodes.size();
        cfg.addEdge(1000, m_m, m,
                1001, m_m, m);
        cfg.addNode(1000, m_m, m);
        CFG.Node n = new CFG.Node(1000, m_m, m);
        assertTrue(cfg.nodes.size() == initialSize + 2);
    }
    ...
    @Test
    public void deleteEdge_multiSrcNode() {
        CFG.Node n1 = new CFG.Node(1000, m_m, m);
        CFG.Node n2 = new CFG.Node(2000, m_m, m);
        CFG.Node nn = new CFG.Node(1001, m_m, m);

        // add an edge
        cfg.addEdge(n1.position, n1.method, n1.clazz, nn.position, nn.method, nn.clazz);
        // add an edge
        cfg.addEdge(n2.position, n2.method, n2.clazz, nn.position, nn.method, nn.clazz);

        int edgeSizeExpected = cfg.edges.get(n1).size();
        int nodeSizeExpected = cfg.nodes.size();

        // proceed if the edge is added successfully
        Assume.assumeTrue(cfg.edges.get(n1).contains(nn));

        // remove the added edge
        cfg.deleteEdge(n1.position, n1.method, n1.clazz, nn.position, nn.method, nn.clazz);
        assertFalse(cfg.edges.get(n1).contains(nn));
        assertEquals(edgeSizeExpected - 1, cfg.edges.get(n1).size());
        assertEquals(nodeSizeExpected - 1, cfg.nodes.size());
    }
